...
export const CartProvider = ({ children }) => {
    ...
    try {
      ...
      transformedItems.forEach((item, index) => {
        ...
        if (duplicateCheck.has(key)) {
          duplicates.push({ index, item, original: duplicateCheck.get(key) });
        } else {
          duplicateCheck.set(key, { index, item });
        }
      });
      
      if (duplicates.length > 0) {
        console.error('🚨 DUPLICATE CART ITEMS DETECTED FROM BACKEND:', duplicates);
        console.error('🚨 This should not happen - backend should prevent duplicates');
      }
      
      // Deduplicate items based on product ID (keep the one with higher quantity)
      ...
      transformedItems.forEach(item => {
        ...
        if (seenProducts.has(productId)) {
          const existingItem = seenProducts.get(productId);
          // Keep the item with higher quantity
          if (item.quantity > existingItem.quantity) {
            const index = deduplicatedItems.findIndex(i => i.product === productId);
            deduplicatedItems[index] = item;
            seenProducts.set(productId, item);
            console.log(`🔄 Replaced duplicate item for product ${productId} with higher quantity (${item.quantity} vs ${existingItem.quantity})`);
          } else {
            console.log(`⚠️ Skipped duplicate item for product ${productId} with lower/equal quantity (${item.quantity} vs ${existingItem.quantity})`);
          }
        } else {
      ...
      if (deduplicatedItems.length !== transformedItems.length) {
        console.warn(`🧹 Removed ${transformedItems.length - deduplicatedItems.length} duplicate items`);
      }
      
      console.log('📦 Setting cart items to state...');
      setCartItems(deduplicatedItems);
      console.log('✅ Cart loaded from backend:', deduplicatedItems.length, 'items');
  ...
  const addToCart = async (product, quantity = 1) => {
    ...
    if (user) {
      ...
      try {
        ...
        console.log('🔍 Duplicate check for optimistic update:', {
          productId: product._id,
          existingItemIndex,
          currentCartLength: cartItems.length,
          existingItem: existingItemIndex >= 0 ? cartItems[existingItemIndex] : null
        });
        ...
        if (existingItemIndex >= 0) {
          ...
          console.log('🔄 Updated existing item in cart optimistically');
        } else {
          // Add new item optimistically
          setCartItems(prev => {
            console.log('➕ Adding new item optimistically to cart with', prev.length, 'existing items');
            return [...prev, newItem];
          });
          console.log('➕ Added new item to cart optimistically');
        }
        
        // Call backend API to sync with server
        const response = await cartService.addToCart(product._id, quantity);
        console.log('✅ Item synced with database cart');
  ...
  const deduplicateCart = () => {
    console.log('🧹 Manually deduplicating cart items...');
    const currentItems = getCurrentCartItems();
    ...
    currentItems.forEach(item => {
      ...
      if (seenProducts.has(productId)) {
        ...
        if (item.quantity > existingItem.quantity) {
          ...
          deduplicatedItems[index] = item;
          seenProducts.set(productId, item);
          console.log(`🔄 Replaced duplicate with higher quantity for product ${productId}`);
        }
      } else {
        deduplicatedItems.push(item);
        seenProducts.set(productId, item);
    ...
    if (user) {
      ...
      setGuestCartItems(deduplicatedItems);
      saveGuestCartToStorage(deduplicatedItems);
    }
    
    console.log(`✅ Deduplication complete: ${currentItems.length} → ${deduplicatedItems.length} items`);
  };
